...
set -a
_main_source_dir="$(dirname "$(readlink -f "$0")")"
function use() {
	local _input="$1";
	local _src_path
	_src_path="$(sed 's|::|/|g' <<<"$_input")"

	source "$_main_source_dir/"$_src_path.sh || exit

# 	if test -d "$_main_source_dir/$_input"; then
# 		source "$_main_source_dir/$_input"/*;
# 	elif test -e "$_main_source_dir/${_input}.sh"; then
# 		source "$_main_source_dir/${_input}.sh";
# 	elif test -e "$_main_source_dir/${_input}"; then
# 		source "$_main_source_dir/${_input}"
# 	else
# 		echo "$_input not found" && exit 1;
# 	fi
}
readonly -f use
set +a
...
function main() {
	...
	### Main execution
	#####################
	_subcommand_argv="$1" && shift;
	case "$_subcommand_argv" in
		new | build | clean | metadata)
