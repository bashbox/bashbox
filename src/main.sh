...
alias use='BB_SOURCE="${BASH_SOURCE[0]}" __use_func';
...
function __use_func() {
	for _input in "${@}"; do {
		...
		if test -e "$_src/${_parsed_input}.sh"; then { # When we have the file in module level.
			...
			} elif fetchLib_fromPath; then { # Try to loopup in declared LIB PATH.
				true
